...
numIter <- 1000
nSamples <- ncol(w)
A.chain <- matrix(NA, numIter+1, (K+1)*nSamples)
A.chain[1, ] <- as.numeric(init.admat(mcmc_w, zero.thresh=0.01))
score.chain <- rep(NA, numIter+1)
numAccept  <-  0
for (i in seq_len(numIter)) {
    if(i %% 100 == 0) cat(".")
    am <- matrix(A.chain[i, ], (K+1), nSamples)
    ##P.prev <- score.admat.chain(am, w.chain)
    mcf <- w.chain[i, , drop=FALSE]
    P.prev <- scoreA(am, w=mcf)
    score.chain[i] <- P.prev
    P.star <- 0
    while (P.star == 0) {
        Astar <- mutateA(am)
        P.star <- scoreA(Astar, mcf)
    }
    r <- P.star / P.prev
    u <- runif(1,0,1)
    if(u <= r) {
        A.chain[i+1, ] <- as.numeric(Astar)
        numAccept <- numAccept + 1
    } else {
        A.chain[i+1, ] <- A.chain[i, ]
    }
}
score.chain[i+1] <- scoreA(matrix(A.chain[i+1, ], (K+1), nSamples), mcf)
