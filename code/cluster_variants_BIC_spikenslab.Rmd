...
outdir <- file.path("..", "output", "cluster_variants_BIC_spikenslab.Rmd")
dir_create(outdir)
```
...
I <- 120; K <- 10; S <- 3
set.seed(123)
input.data <- simulateData3(I, K, S)
saveRDS(input.data, file.path(outdir, "input.data.rds"))
```
...
for(i in 1:length(kToTest)) {
  samps.list[[i]] <- runMCMC(input.data, kToTest[i], jags.file, inits, params, n.iter, thin, n.adapt = 1000)
}
