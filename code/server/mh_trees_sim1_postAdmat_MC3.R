...
option_list = list(
  ...
  make_option(c("-p", "--post.admat"), type="character", default=NULL,
              help="path to posterior adjacency matrix tsv file", metavar="character")
)
...
if (is.null(opt$post.admat)){
  print_help(opt_parser)
  stop("Posterior adjacency matrix must be supplied", call.=FALSE)
}
...
# ===================================================================================== #
# Posterior Adjacency Matrix

post.admat <- read.table(opt$post.admat, 
...
init_admat <- restrictions.from.post.admat(post.admat)
...
for (i in seq_len(opt$numIter)) {
    ...
    fit.star <- calc.tree.fitness(admat.star, cpov.star, sampled.w.star, weight.mass=3)
