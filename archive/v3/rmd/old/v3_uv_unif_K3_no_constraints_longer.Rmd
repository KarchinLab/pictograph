...
runName <- "v3_K3_noConstraints_longer"
```
...
jags.m <- jags.model(jags.file, test.data,
                     ...
                     inits = list(".RNG.name" = "base::Wichmann-Hill", 
                                  ".RNG.seed" = 123))
params <- c("z", "w", "U", "V")
samps <- coda.samples(jags.m, params, n.iter=15000, thin=7)
s <- summary(samps)
effectiveSize(samps)
...
for (s in 1:S) {
  for (k in 1:K) {
    if (k == 1) {
      ...
      legend(x = "topleft", 
             legend = paste0(c("mcmc k", "true k"), rep(1:K, each=2), ", U=", allU),
...
posterior <- as.array(samps)

U.ordered[,1]
mcmc_dens(posterior, pars = "U[1,1]")
mcmc_dens(posterior, pars = "U[2,1]")
mcmc_dens(posterior, pars = "U[3,1]")
