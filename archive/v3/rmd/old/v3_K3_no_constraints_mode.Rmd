...
runName <- "v3_K3_noConstraints_mode"
```
...
mcmc_d <- mcmc_vals[substr(rownames(mcmc_vals), 1, 1) == "d", "Mean"]
mcmc_d <- matrix(mcmc_d, nrow=K)
mcmc_c <- mcmc_vals[substr(rownames(mcmc_vals), 1, 1) == "c", "Mean"]
...
for (s in 1:S) {
  for (k in 1:K) {
    if (k == 1) {
      # plot mcmc mean 
      plot(p, dbeta(p, 
                    mcmc_d[k,s] * (mcmc_c[k,s] - 2) + 1, 
                    (1 - mcmc_d[k,s]) * (mcmc_c[k,s] - 2) + 1), 
           main = paste0("S", s), 
      ...
      # add legend
      allU <- round(as.vector(rbind(mcmc_d[,s], U.ordered[,s])), digits = 2)
      legend(x = "topleft", 
      ...
      # plot mcmc mean U,V
      lines(p, dbeta(p, mcmc_d[k,s] * (mcmc_c[k,s] - 2) + 1, 
                    (1 - mcmc_d[k,s]) * (mcmc_c[k,s] - 2) + 1), 
            type = "l", col = colors[k])
