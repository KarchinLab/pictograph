...
ggplot(w_comp2, aes(sample, omega, group=grp)) +
    ...
    theme(axis.text.x=element_text(angle=45, hjust=1),
          ...
          legend.key=element_rect(fill="white", color="white")) +
    guides(color=guide_legend(title="Variant cluster\n(z)"))
ggsave(file.path(working.dir, paste0(runName, "_wLine.pdf")), height=4, width=6)
```

```{r betas, echo=F, fig.height=10, fig.width=8}
...
w_chains2 <- w_chains %>%
  as_tibble() %>%
  gather("chain", "w") 
w_chains2$pos <- substr(w_chains2$chain, 3, nchar(w_chains2$chain)-1)
w_chains2$cluster <- sapply(strsplit(w_chains2$pos, ","), `[[`, 1)
z_mapping2 <- z_mapping[order(z_mapping$mcmc_z), ]
w_chains2$cluster <- z_mapping2$z[as.numeric(w_chains2$cluster)]
w_chains2$cluster <- as.character(w_chains2$cluster)
w_chains2$sample <- paste0("sample", sapply(strsplit(w_chains2$pos, ","), `[[`, 2))
w_chains2$type <- "w post"
w_chains2$grp <- paste0("k", w_chains2$cluster, ",", w_chains2$sample, ",", w_chains2$type)

## beta distributions by sample
beta.plot <- ggplot() +
  geom_line(data=dens_df,
            aes(x=x, y=val, group=grp, color=cluster, lty=type)) +
  facet_wrap(~sample, ncol=3) +
  ylab("density") +
  xlab(expression(omega)) +
  theme(panel.background=element_rect(fill="white", color="black"))
beta.plot
ggsave(file.path(working.dir, paste0(runName, "_betas.pdf")), height=6, width=8)
```

```{r beta2, echo=F, fig.height=10, fig.width=8}
beta.plot2 <- beta.plot +
    geom_density(data=w_chains2,
                 aes(x=w, fill=cluster), alpha=0.3, position="identity") +
  ggtitle("w betas + posterior")
beta.plot2
```
