...
seed <- 1
weight.mass <- 4
run_name <- paste0("K", K, "_seed", seed, "_startAtGuess_", weight.mass, "MC")
tree_rds <- paste0("trees_50k_notThinned_", run_name, ".rds")
results <- readRDS(file.path("..", "output", "IP30_weighted_mutate.Rmd", tree_rds))
...
ggsave(file.path(outdir, paste0("log_score_chain_", run_name, ".pdf")), width = 7, height = 4)
...
ggsave(file.path(tree.plot.dir, paste0("top.10.score.trees.", run_name, ".pdf")), 
       score.plots, width = 15, height = 20)

freq.plots <- do.call(grid.arrange, c(top.freq.plots, nrow=4))
ggsave(file.path(tree.plot.dir, paste0("top.10.freq.trees.", run_name, ".pdf")), 
