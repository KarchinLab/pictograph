...
ggplot(p2, aes(variant_index, fraction)) +
    ##geom_jitter(width=0.15, size=2, aes(color=multiplicity)) +
    geom_point(size=2) +
    facet_wrap(~sample) +
    ylab("y/n") +
...
ggplot(ppd.summaries, aes(x=variant_index,
                          y=mean,
                          ymin=q1,
                          ymax=q3)) +
    geom_errorbar(color="gray") +
    geom_point(aes(variant_index, observed),
...
mcmc_z %>%
    ...
    mutate(sim_cluster=paste0("Clone ", sim_cluster),
           sim_cluster=factor(sim_cluster, levels=paste0("Clone ", 1:K))) %>%
...
mcmc_vals <- w.chain %>%
        ...
        summarize(mean=mean(value),
                  q1=quantile(value, 0.025),
                  q3=quantile(value, 0.975)) %>%
        mutate(truth=as.vector(t(input.data$w))) 
...
ggplot(mcmc_vals, aes(mean, truth)) +
    ...
    theme(axis.text=element_text(size=15),
          panel.background=element_rect(fill="white",
                                        color="black"),
          axis.title=element_text(size=17)) +
ggsave(file.path(outdir, "MCF.pdf"), width=10, height=7)
```
