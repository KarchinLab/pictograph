...
# True Tree
```{r}
w.chain <- readRDS(file.path("..", "output", "cluster_variants_BIC_I120.Rmd", "K10_w_chain_relabeled.rds"))
mcf_stats <- w.chain %>%
    group_by(Parameter) %>%
...
answer["root", "cluster1"] <- answer["cluster1", "cluster2"] <-
    answer["cluster2", "cluster3"] <- answer["cluster2", "cluster6"] <-
    answer["cluster3", "cluster4"] <- answer["cluster3", "cluster5"] <-
    answer["cluster4", "cluster8"] <- answer["cluster5", "cluster9"] <-
    answer["cluster6", "cluster7"] <- answer["cluster7", "cluster10"] <- 1
...
for (i in 1:10) {
  ...
  dag <- plotDAG(adm) + 
    ggtitle(paste0("Rank ", i, 
                   ...
                   ", prop_true = ", round(calc.tree.prop.true(adm, answer), 2)))
