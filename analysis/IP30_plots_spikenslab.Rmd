...
z.p.chain <- chains[grep("z.p\\[", chains$Parameter), ]
p.chain <- chains[grep("p\\[", chains$Parameter), ]
...
muts <- y$mut_id
muts.z <- map_z %>%
  mutate(Mutation = muts) %>%
  arrange(value) %>%
  rename(Cluster = value)
write.table(muts.z, file.path(outdir, paste0("mutations_map_z_K", K, ".tsv")),
            sep = "\t", quote = F, col.names = T, row.names = F)
```

# Mean w
```{r}
mcf_matrix <- round(mcfMatrix(mcf_stats),2)
colnames(mcf_matrix) <- colnames(input.data$y)
write.table(mcf_matrix,
            file.path(outdir, paste0("mean_mcf_matrix_K", K, ".tsv")),
            sep = "\t", quote = F, col.names = T, row.names = F)
```
...
```{r thin}
results <- readRDS(file.path("..", "output", "IP30_spikenslab_2.Rmd", 
                             paste0("trees_50k_notThinned_K", K, ".rds")))
...
ggplot(thinned.tb, aes(x = iter, y = log.score)) +
  geom_line() +
  theme_light()
ggsave(file.path(outdir, paste0("log_score_chain_K", K, ".pdf")), width = 7, height = 4)
...
ggsave(file.path(tree.plot.dir, paste0("top.10.score.trees.K", K, ".pdf")), 
       score.plots, width = 15, height = 20)

freq.plots <- do.call(grid.arrange, c(top.freq.plots, nrow=4))
ggsave(file.path(tree.plot.dir, paste0("top.10.freq.trees.K", K, ".pdf")), 
