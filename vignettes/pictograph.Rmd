...
BIC values for all $K$ assessed in each mutation set can be visualized using `plotBIC`. 

```{r plot_bic, height = 5, width = 8}
...
The posterior distribtuion of cluster CCFs can be visualized as violin plots

```{r ccf_violin, fig.height = 4, fig.width = 6} 
plotCCFViolin(chains$w_chain, input_data)
```
...
scores <- calcTreeScores(chains$w_chain, filtered_trees)
scores
