...
dir.create(figs.dir, showWarnings = F)
```
...
#   answer[from, to] <- 1
# }
answer[1,1] <- answer[2,2] <- answer[5,3] <- answer[9,4] <- answer[10,5] <- answer[11,6] <-answer[3,7] <- answer[3,8] <- answer[8,9] <- answer[9,10] <- 1
answer
...
score.chain <- calc.tree.fitness(admat.chain[[1]], cpov, mcmc_w)
...
score.chain[numIter+1] <- calc.tree.fitness(admat.chain[[numIter+1]], cpov, mcmc_w)
acceptRate <- numAccept/(numIter)
acceptRate
...
```{r num_trees, eval=F, echo=F} 
restrictions <- base.admat(mcmc_w)

calc_possible_trees_restrictions <- function(restrictions) {
  prod(apply(restrictions, 2, function(x) sum(!is.na(x))))
}

formatC(calc_possible_trees_restrictions(restrictions))

calc_all_trees <- function(K) {
  total_possible <- (K+1)^K
  no_root <- K^K
  actual_trees <- total_possible - no_root
  actual_trees
}

formatC(calc_all_trees(K))
```